...
def main():
    ...
    print(
        f"  Income range: ${original_df['income'].min():,.0f}-${original_df['income'].max():,.0f}"
    )
    ...
    preset_choice = (
        "ml_training"  # Change to: gdpr_compliant, ml_training, vendor_sharing
    )
    ...
    try:
        ...
        compare_before_after(
            original_df, anonymized_df, ["email", "age", "income", "phone", "ssn"]
        )
    ...
    try:
        ...
        for col in numeric_cols:
            try:
                ...
                print(f"\n{col}:")
                print(
                    f"  KS Statistic: {dist_metrics.ks_statistic:.4f} (lower is better)"
                )
        ...
        try:
            ...
            print(
                f"Mean Absolute Difference: {corr_metrics.mean_absolute_difference:.4f}"
            )
        ...
        for col in original_df.columns[:6]:  # Sample first 6 columns
            try:
                ...
                print(f"\n{col}:")
                print(
                    f"  Unique values: {info_metrics.unique_values_original} → "
                    f"{info_metrics.unique_values_anonymized} "
                    f"({info_metrics.unique_values_retained_pct:.1f}% retained)"
                )
                print(
                    f"  Entropy: {info_metrics.entropy_original:.2f} → "
