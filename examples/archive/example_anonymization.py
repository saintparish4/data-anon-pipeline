...
def example_combined_techniques():
    ...
    df_anon["email"] = df["email"].apply(
        lambda x: anonymizer.redaction.redact_partial_email(x)
    )
    df_anon["ssn"] = df["ssn"].apply(
        lambda x: anonymizer.redaction.redact_partial_ssn(x)
    )
    df_anon["age"] = df["age"].apply(anonymizer.generalize_age)
    df_anon["zipcode"] = df["zipcode"].apply(anonymizer.generalize_zipcode)
    df_anon["dob"] = df["dob"].apply(
        anonymizer.generalization.generalize_date_to_quarter
    )
...
def example_technique_selection():
    ...
    print(
        f"    Phone → Fake: {anonymizer.pseudonymization.pseudonymize_phone('(555) 123-4567')}"
    )
    ...
    print("  Solution:")
    print(
        f"    SSN → Partial: {anonymizer.redaction.redact_partial_ssn('123-45-6789')}"
    )
    print(
        f"    Card → Partial: {anonymizer.redaction.redact_partial_credit_card('4532-1488-0343-6467')}"
    )
