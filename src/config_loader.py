...
class AnonymizationStrategy(Enum):
    ...
    PRESERVE = "preserve"  # Keep data unchanged
    ADD_NOISE = "add_noise"  # Add differential privacy noise
...
class RuleConfig:
    ...
    def _validate_generalize_parameters(self) -> None:
        ...
        valid_sets = [
            {"bin_size", "min_value", "max_value"},  # Numeric binning (full)
            {"bin_size"},  # Numeric binning (minimal - min/max can be inferred)
            {"precision"},  # Location data (zipcode, IP)
            {"granularity"},  # Date/time data
            {"level"},  # Address/hierarchical data
            {"octets"},  # IP address specific
            {"method", "subnet_mask"},  # IP address subnet notation (from presets)
            {"method", "keep_digits"},  # Zipcode truncation (from presets)
            {"method"},  # Generic method-based config (from presets)
        ]
...
class ConfigLoader:
    ...
    def _validate_schema(self) -> None:
        ...
        # Support both 'rules' and 'anonymization_rules' for backwards compatibility
        if (
            "anonymization_rules" in self._raw_config
            and "rules" not in self._raw_config
        ):
            # Normalize: copy anonymization_rules to rules
            self._raw_config["rules"] = self._raw_config["anonymization_rules"]
    ...
    def _normalize_parameters(
        self, strategy: AnonymizationStrategy, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Normalize parameter names for backwards compatibility.

        Args:
            strategy: The anonymization strategy
        ...
        # For pseudonymize: map 'consistent' to 'seed_based'
        if strategy == AnonymizationStrategy.PSEUDONYMIZE:
            if "consistent" in params and "seed_based" not in params:
                params["seed_based"] = params.pop("consistent")
            # Ensure seed_based exists (default to True)
            if "seed_based" not in params:
                params["seed_based"] = True
    ...
    def _parse_rules(self) -> None:
        ...
        for pii_type, rule_data in rules_dict.items():
            if not isinstance(rule_data, dict):
                raise ConfigurationError(f"Rule for '{pii_type}' must be a dictionary")

            # Support both 'strategy' and 'technique' for backwards compatibility
            strategy_str = rule_data.get("strategy") or rule_data.get("technique")
            if not strategy_str:
                raise ConfigurationError(
                    f"Rule for '{pii_type}' missing 'strategy' or 'technique' field"
                )
            ...
            # Support both 'parameters' and 'params' for backwards compatibility
            parameters = rule_data.get("parameters") or rule_data.get("params", {})

            # Normalize parameters for backwards compatibility
            parameters = self._normalize_parameters(strategy, parameters)

            rule = RuleConfig(
                pii_type=pii_type, strategy=strategy, parameters=parameters
            )
