...
class InformationLossMetrics:
    ...
    def __post_init__(self):
        """Add interpretation based on metrics."""
        avg_retention = (self.unique_values_retained_pct + self.entropy_retained_pct) / 2
...
class UtilityReport:
    ...
    information_loss_metrics: Dict[str, InformationLossMetrics] = field(default_factory=dict)
...
class UtilityMetrics:
    ...
    def calculate_correlation_preservation(self) -> CorrelationMetrics:
        ...
        try:
            ...
            # Handle edge case where std dev is zero
            corr_dist = 0.0
    ...
    def calculate_information_loss(
        ...
        unique_retained_pct = (
            ...
            if unique_original > 0 else 100.0
        )
        ...
        entropy_retained_pct = (
            ...
            if entropy_original > 0 else 100.0
        )
    ...
    def generate_report(
        ...
        for metrics in report.information_loss_metrics.values():
            avg_retention = (
                metrics.unique_values_retained_pct +
                metrics.entropy_retained_pct
            ) / 2
    ...
    def _generate_recommendations(self, report: UtilityReport) -> List[str]:
        ...
        poor_dist_cols = [
            col for col, metrics in report.distribution_metrics.items()
        ...
        high_loss_cols = [
            col for col, metrics in report.information_loss_metrics.items()
...
if __name__ == "__main__":
    ...
    anonymized['age'] = (anonymized['age'] // 10 * 10).astype(str) + "-" + ((anonymized['age'] // 10 * 10) + 9).astype(str)
