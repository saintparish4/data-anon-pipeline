...
class PrivacyValidator:
    ...
    def validate(
        self, df_anonymized: pd.DataFrame, df_original: Optional[pd.DataFrame] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _check_k_anonymity(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not passed and allow_outliers:
            ...
            if outlier_percent <= max_outlier_percent:
                passed = True
                message = (
                    f"k-anonymity {min_k} below threshold {required_k}, "
    ...
    def _check_l_diversity(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        quasi_identifiers = self.thresholds.get("k_anonymity", {}).get(
            "quasi_identifiers", []
        )
    ...
    def _check_reidentification_risk(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        df_temp = df.copy()
        df_temp["_group_size"] = df_temp.groupby(available_qi, dropna=False)[
            available_qi[0]
        ].transform("count")
        ...
        medium_risk_count = len(
            df_temp[(df_temp["_group_size"] >= 3) & (df_temp["_group_size"] <= 4)]
        )
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        if results["passed"]:
            ...
            failed_checks = [
                name for name, check in results["checks"].items() if not check["passed"]
            ]
    ...
    def get_detailed_report(
        ...
        for check_name, check_result in results["checks"].items():
            ...
            if check_name == "k_anonymity":
                ...
                report_lines.append(
                    f"  Average k: {check_result.get('avg_k', 'N/A'):.2f}"
                )
                report_lines.append(
                    f"  Required k: {check_result.get('required_k', 'N/A')}"
                )

            elif check_name == "l_diversity":
                if check_result.get("min_l") is not None:
                    report_lines.append(
                        f"  Minimum l: {check_result.get('min_l', 'N/A')}"
                    )
                    report_lines.append(
                        f"  Average l: {check_result.get('avg_l', 'N/A'):.2f}"
                    )
                    report_lines.append(
                        f"  Required l: {check_result.get('required_l', 'N/A')}"
                    )

            elif check_name == "reidentification_risk":
                report_lines.append(
                    f"  High risk: {check_result.get('high_risk_percent', 0):.1f}% "
                    f"({check_result.get('high_risk_count', 0)} records)"
                )
                report_lines.append(
                    f"  Medium risk: {check_result.get('medium_risk_percent', 0):.1f}% "
                    f"({check_result.get('medium_risk_count', 0)} records)"
                )
                report_lines.append(
                    f"  Low risk: {check_result.get('low_risk_percent', 0):.1f}% "
